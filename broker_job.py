...
def main() -> int:
    ...
    try:
        ...
        # A broker HTTP status is safe operational telemetry. Keep every other
        # exception type-only so source URLs and filenames remain private.
        message = str(exc)
        detail = message if message.startswith("Broker returned HTTP ") else type(exc).__name__
        print(f"Broker transfer failed: {detail}", file=sys.stderr)
        return 1
